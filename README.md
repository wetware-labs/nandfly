...
The deployed contract is real, live, and swattable today — but it is not yet **born** in the sense this project means it. Birth is a public on-chain event: a 7-cell fragment of this circuit (6 NAND + 1 LATCH), assembled from real components in the [TapeOut](https://tapeout.net) NAND-gate fab — an on-chain component system, not physical silicon — and minted publicly once the community funds it. (TapeOut's fab contract is third-party and unverified on BscScan; our mint path was reverse-engineered and cross-checked against two live circuits — see `tapeout/SUBMISSION.md` for exactly what is and isn't certain.)

- **Goal: $30** (the current fab-quoted cost of the 7-cell mint).
- **Birth certificate:** the wallet whose inflow crosses the goal (by on-chain record) is credited permanently — on the site, in this repo, and in the public birth-mint transaction. One wallet, one name, forever; currently **unclaimed**. Credit, not yield.
