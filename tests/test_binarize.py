...
def test_full_netlist_jump_fires_when_enough_visual_drive_present():
    ...
    # left LC4(60)+LPLC2(30): quantized 10+5=15, threshold=ceil(0.5*15)=8;
    # both firing (15) clears threshold; GF fires -> TTMn(weight20->q3) fires
    # -> latch sets.
...
def test_extract_seed_fragment_is_verbatim_subset_of_full():
    ...
    for g in seed["gates"]:
        assert full_gates_by_id[g["id"]] == g  # byte-identical to the full netlist's gate
