...
export async function renderBirthGauge(container) {
  ...
  try {
    ...
    renderBar(
      `$${raisedUsd.toFixed(2)}`,
      pct,
      "Live from BSC: the feeding wallet's balance minus the disclosed gas float (~0.0014 BNB, our own deploy gas, not donations), converted at Chainlink's on-chain BNB/USD feed. No manual number anywhere."
    );
  } catch (e) {
